...
/*#__PURE__*/
function () {
  function SearchLetter(input, letter_body, letters) {
  ...
  _createClass(SearchLetter, [{
    ...
    value: function search(e) {
      ...
      if (e.keyCode >= 48 && e.keyCode <= 57 || e.keyCode >= 65 && e.keyCode <= 90 || e.keyCode == 8) {
        ...
        if (e.target.value == "") {
          this.appendItems(this.letter_body, this.letters);
        }

        this.searched = this.letters.filter(function (letter) {
          return letter.dataset.reference.indexOf(_this.input) !== -1;
        });
        this.letter_body.innerHTML = "";
        this.appendItems(this.letter_body, this.searched); // this.animChange()

        if (document.querySelector(".filter--cnt-sum .sum strong")) {
          document.querySelector(".filter--cnt-sum .sum strong").innerHTML = this.searched.length;
        }
      }
    }
  }, {
